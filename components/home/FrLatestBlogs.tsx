...
export default function FrLatestBlogs() {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-[#A0A0A0] text-lg max-w-2xl mx-auto">
            Guides, conseils et tutoriels de configuration pour tirer le meilleur parti de votre abonnement Varodatic IPTV Suisse.
          </p>
        </div>
          ...
          {featured.map((post) => (
            ...
            >
              <div className="relative w-full aspect-video overflow-hidden">
                ...
                <span className="absolute top-3 left-3 text-xs font-bold uppercase tracking-widest text-black bg-primary px-3 py-1 rounded-full">{post.tag}</span>
              </div>
              <div className="p-6">
                <div className="flex items-center gap-3 text-[#A0A0A0] text-xs mb-3">
                  <span>{new Date(post.date).toLocaleDateString("fr-CH", { year: "numeric", month: "short", day: "numeric" })}</span>
                  <span>·</span>
                  <span>{post.readTime}</span>
                </div>
                <h3 className="text-white font-bold text-lg mb-3 leading-snug group-hover:text-primary transition-colors duration-300">{post.title}</h3>
        ...
        <div className="text-center mt-12">
          <Link href="/blog" className="inline-block px-8 py-3 border-2 border-primary/40 text-primary font-semibold rounded-xl text-sm hover:border-primary hover:bg-primary/10 transition-all duration-300">
